...
def train_yolov8_segmentation_simple(model_size='n', epochs=150, imgsz=640, batch=-1, patience=20, resume=False):
    """
    简洁版YOLOv8分割模型训练
    ...
    if resume:
        # 从上次训练中断处继续训练
        last_weights_path = PROJECT_ROOT / 'runs' / 'segment' / run_name / 'weights' / 'last.pt'
        if not last_weights_path.exists():
            print(f"错误: 找不到上次训练的权重文件: {last_weights_path}")
            return None, None
        model_path = str(last_weights_path)
        print(f"从上次训练中断处继续训练: {model_path}")
    else:
        # 从预训练模型开始训练
        model_path = PROJECT_ROOT / 'models' / 'seg' / model_map[model_size]
        print(f"从预训练模型开始训练: {model_path}")
    ...
    try:
        results = model.train(
            ...
            device=0,  # 使用GPU
            amp=False,
            resume=resume  # 关键参数：是否从上次训练中断处继续
        )
        
        print(f"\n训练完成! 最佳模型保存在: {PROJECT_ROOT / 'runs' / 'segment' / run_name}")
